...
[begin_points, end_points,ls_likelihoods,lineIndex] = get_all_segments_assoc_edgeremoval( P, edgeStruct, pp, LIKE_MODEL, scale_factor);

ttlLS = size(begin_points,1);
out = zeros(ttlLS,6);  
for dd = 1:ttlLS 
    out(dd,:) = [begin_points(dd,1), begin_points(dd,2), end_points(dd,1), end_points(dd,2),ls_likelihoods(dd),lineIndex(dd)];         
end
