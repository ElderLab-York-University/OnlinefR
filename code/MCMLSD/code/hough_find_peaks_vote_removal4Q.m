...
%
%
function [P] = hough_find_peaks_vote_removal4Q(map, num, r_res, th_res, edgeStruct, kernels, kernels_flip, pp, kernel_params, sig_th, sig_r, sig_bound, frac, scale_factor)
    ...
    while 1
       ...
       maxmap4Q=zeros(4,1);
       ind4Q=zeros(4,2);
       [maxmapT,indT]=max(map(1:785,1:2000));
       [a,b]=max(maxmapT);
       ind4Q(1,:)=[indT(b),b];
       maxmap4Q(1)=a;
       
       [maxmapT,indT]=max(map(786:end,2001:end));
       [a,b]=max(maxmapT);
       ind4Q(2,:)=[indT(b)+785,b+2000];
       maxmap4Q(2)=a;
       
       [maxmapT,indT]=max(map(786:end,1:2000));
       [a,b]=max(maxmapT);
       ind4Q(3,:)=[indT(b)+786,b];
       maxmap4Q(3)=a;
       
       [maxmapT,indT]=max(map(1:785,2001:end));
       [a,b]=max(maxmapT);
       ind4Q(4,:)=[indT(b),b+2000];
       maxmap4Q(4)=a;
       threshold4Q=max(maxmap4Q);
       edge_img_locations_removeT=zeros(10000,2);
       edge_gradients_removeT=zeros(10000,1);
       countT=1;
       for i=1:4
...
%         edge_index = edge_index(distanceBoundary);
        ...
        edge_img_locations_removeT(countT:countT+size(edge_img_locations_remove,1)-1,:)=edge_img_locations_remove;
        edge_gradients_remove = edge_gradients_remove(orientatinDiffboundary);
        edge_gradients_removeT(countT:countT+size(edge_gradients_remove,1)-1,:)=edge_gradients_remove;
        %edge_index = edge_index(orientatinDiffboundary);
        countT=countT+size(edge_img_locations_remove,1);
        ...
        map=mexRemoveVotes_v3_scale(edge_img_locations_removeT(1:countT,:), edge_gradients_removeT(1:countT), kernels, kernels_flip, r_res, th_res, pp, kernel_params, map, scale_factor);
...
%         imwrite(edgeMap,['img' num2str(iter) '.png'])
    end
end
