...
bottom = n - y0;
lineWidth=3;
%Identify pixels within 4 pixels of line
maxpts = round(10*sqrt(m^2+n^2));
...
if theta > pi/4 && theta < 3*pi/4
    x = left:right;
    yu = floor(min(bottom,(-c+lineWidth - x*a)/b));
    yl = ceil(max(top,(-c-lineWidth - x*a)/b));
    for i = 1:m
    ...
    s = sign(cos(theta)); 
    xu = floor(min(right,(-c+lineWidth*s - y*b)/a));
    xl = ceil(max(left,(-c-lineWidth*s - y*b)/a));
    for i = 1:n
